...
void gen(Node *node)
{
  ...
  if (!node) error_exit_with_guard("null pointer");
  ...
  switch (node->kind)
  {
    ...
    case ND_DISCARD_EXPR:
      ...
      gen(node->lhs);
      if (node->kind == ND_DISCARD_EXPR) output_file("    add rsp, 8");
      return;
    ...
    case ND_FOR:
    {
      ...
      if (node->kind == ND_FOR) gen(node->init);
      ...
      gen(node->true_code);
      if (node->kind == ND_FOR) gen(node->update);
...
void generator(FuncBlock *parsed, char *output_filename)
{
  ...
  for (FuncBlock *pointer = parsed; pointer; pointer = pointer->next)
  {
    ...
    if (node->kind != ND_VAR) error_exit_with_guard("unreachable");
  }
